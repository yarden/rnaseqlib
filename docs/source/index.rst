...
1. **Run** MISO on samples: ::
  ...
  misowrap.py --run misowrap_settings.txt --output-dir miso_output/

2. Once run is completed, **summarize samples**: ::

  misowrap.py --summarize misowrap_settings.txt --output-dir miso_output/
...
pairwise comparisons to run. ::
  ...
  misowrap.py --compare misowrap_settings.txt --output-dir miso_output/

4. **Combine comparisons** into single files (does not apply any filters) and place them in the directory where our comparisons are. Takes the ``comparisons`` directory (which contains MISO comparisons) and the settings file ::

  misowrap.py --combine-comparisons miso_output/comparisons/ miso_settings.txt --output-dir miso_output/comparisons/

This creates single files that contain information about events pooled from all the samples described in the settings file.


5. **Filter comparisons** to create a set of events that are filtered for meeting the read count filters described in the settings file, and output filtered comparisons to the comparisons directory ::

  misowrap.py --filter misowrap_settings.txt --output-dir miso_output/comparisons/

This creates a directory ``filtered_events/`` in ``miso_output/comparisons/`` that contains the filtered ersions of each event file.

6. **Combine filtered comparisons** to pool filered events into single files: ::
